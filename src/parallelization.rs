...
pub fn pool() -> Vec<Node> {
    ...
    //println!("> {} threads avaible", num_threads);
    ...
    let table: Vec<Node> = pool.install(|| {
        (0..num_threads).into_par_iter()
            .map(|i| {
                ...
                generate_table(start, end, starting_items_shared.clone(), &bar)
            })
            .flatten().collect()
    });
    table
...
fn generate_table( startpassword : u32, endpassword : u32, starting_items_shared : Arc<Mutex<Vec<String>>>, bar: &ProgressBar) -> Vec<Node> {
